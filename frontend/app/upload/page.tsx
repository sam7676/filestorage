  ...
  useEffect(() => {
    ...
    const processNext = async () => {
      ...
      try {
        ...
        setImageQueue((prev) =>
          prev.map((item) =>
            item.previewUrl === next.previewUrl ? { ...item, status: 'pending' } : item,
          ),
        );
        ...
        try {
          ...
          if (!res.ok) {
            setImageQueue((prev) =>
              prev.map((item) =>
                item.previewUrl === next.previewUrl ? { ...item, status: 'failed' } : item,
              ),
            );
        ...
        setImageQueue((prev) =>
          prev.map((item) =>
            item.previewUrl === next.previewUrl ? { ...item, status: 'processed' } : item,
          ),
        );

        setImageQueue((prev) => prev.filter((item) => item.status != 'processed'));
      } catch (err) {
          ...
          <div key={i} style={{ textAlign: 'center' }}>
            ...
            <p style={{ marginTop: '0.5rem' }}>
              {(() => {
                switch (item.status) {
                  case 'processed':
                    return <StatusButton label="✅ Processed" i={i} />;
                  case 'pending':
                    return <StatusButton label="⏳ Pending" i={i} />;
                  case 'unprocessed':
                    return <StatusButton label="📭 Unprocessed" i={i} />;
                  case 'failed':
                    return <StatusButton label="❌ Failed" i={i} />;

                  default:
                    return <StatusButton label="❔ Unknown status" i={i} />;
                }
              })()}
            </p>
          </div>
