...
const CONDITIONS_OPTIONS = ['is', 'is not', 'contains', 'does not contain', 'is null', 'is not null'];
...
export default function View() {
    ...
    const swipeHandlers = useSwipe({ onSwipedLeft: () => loadMedia('video'), onSwipedRight: () => loadMedia('image') });
    ...
    function createImage() {
        var pixels = [
            [loading ? loadingColor : backgroundColor],
        ]
        try {
    ...
    return (
        <div {...swipeHandlers} className="min-h-screen bg-gray-100 p-8 flex items-center justify-center" style={{
            ...
            <div className="w-full">
                <div id="media-container" className="bg-white rounded-lg shadow-lg p-4 justify-center">
                    ...
                    <img src={createImage()} width={1} height={getRemainingHalfHeight(currentHeight)} className="max-w-full h-auto" />

                    {currentUrl && currentType === 'image' && (
                        <img src={currentUrl} width={currentWidth} height={currentHeight} alt="Loaded media" className="max-w-full h-auto" onClick={() => loadMedia('image')} />
                    )}

                    {currentUrl && currentType === 'video' && (
                        <video src={currentUrl} width={currentWidth} height={currentHeight} controls loop autoPlay className="max-w-full h-auto " />
                    )}
                ...
                >
                    ...
                    <button type="button" onClick={() => redirect('/?upload', RedirectType.push)} style={{
                        width: `${getHalfWidth(currentWidth)}px`,
                        backgroundColor: loading ? loadingColor : backgroundColor,
                    }}>Upload</button>
                    <button type="button" onClick={() => redirect('/view', RedirectType.push)} style={{
                        width: `${getHalfWidth(currentWidth)}px`,
                        backgroundColor: loading ? loadingColor : backgroundColor,
                    }}>View</button>
                    <br></br>
